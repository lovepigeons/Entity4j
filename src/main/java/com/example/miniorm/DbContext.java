...
public class DbContext implements AutoCloseable {
    ...
    public <T> Query<T> from(Class<T> type) {
        return new Query<>(this, TableMeta.of(type));
    }
    ...
    public <T> String dropTableSql(Class<T> type, boolean ifExists) {
        TableMeta<T> m = TableMeta.of(type);
    ...
    private static void setField(Object target, Field f, Object val) {
        try { f.setAccessible(true); f.set(target, convert(val, f.getType())); }
        catch (IllegalAccessException e) { throw new RuntimeException(e); }
    }
    ...
    private static Object convert(Object val, Class<?> targetType) {
        ...
        if (targetType == LocalDateTime.class && val instanceof java.sql.Timestamp) return ((Timestamp)val).toLocalDateTime();
        return val;
    }
    ...
    private <T> T mapRow(ResultSet rs, TableMeta<T> m) throws SQLException {
        try {
            ...
            for (Map.Entry<String, String> e : m.propToColumn.entrySet()) {
                ...
                Object dbVal;
                try { dbVal = rs.getObject(col); } catch (SQLException ex) { continue; }
...
/* ---------------------------
   ...
   --------------------------- */
    ...
    String q(String ident) { return dialect.q(ident); }

    /* package-private access for Query */
    Connection conn() { return connection; }
    SqlDialect dialect() { return dialect; }
}
