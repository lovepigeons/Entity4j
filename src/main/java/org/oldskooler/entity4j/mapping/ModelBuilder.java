...
public final class ModelBuilder {
    ...
    static final class ColumnBuilder implements ColumnOptions {
        ...
        public ColumnMeta toMeta() {
            return new ColumnMeta(property, name, nullable, type, precision, scale);
        }
    }
    ...
    public static final class EntityBuilder<T> {
        ...
        /** Primary key property + whether it's auto generated (identity/serial). */
        public EntityBuilder<T> hasId(String property, boolean autoGenerated) {
            this.idProp = property; this.idAuto = autoGenerated;
            if (this.idCol == null) this.idCol = property;
            propToColumn.putIfAbsent(property, this.idCol);
            // default ColumnMeta for id unless overridden later
            columnMeta.putIfAbsent(property, new ColumnBuilder(property).name(this.idCol).nullable(false).toMeta());
            return this;
        }

        /** Override the PK column name. */
        public EntityBuilder<T> idColumn(String column) {
            this.idCol = column;
            if (this.idProp != null) {
                propToColumn.put(this.idProp, column);
                // keep id non-nullable by default
                columnMeta.put(this.idProp, new ColumnBuilder(this.idProp).name(column).nullable(false).toMeta());
            }
            return this;
        }
        ...
        public EntityBuilder<T> map(String property, String column) {
            ...
            columnMeta.putIfAbsent(property, new ColumnBuilder(property).name(column).toMeta());
            return this;
        }

        /** Map property to a same-named column (no extra options). */
        public EntityBuilder<T> mapSame(String property) {
            propToColumn.put(property, property);
            columnMeta.putIfAbsent(property, new ColumnBuilder(property).toMeta());
            return this;
        }
        ...
        public EntityBuilder<T> column(String property, Consumer<ColumnOptions> config) {
            ...
            columnMeta.put(property, cb.toMeta());
            return this;
        }
        ...
        public ModelBuilder done() {
            ...
            reg.register(new EntityMapping<>(type, table, idProp, idCol, idAuto, propToColumn, columnMeta));
